...
public interface CourseRepository extends JpaRepository<Course, Long>{
    ...
    @Query(nativeQuery = true, value=
        "SELECT * FROM tb_course WHERE featured = true;")
    List<Course> findFeaturedCourses();

    @Query(nativeQuery = true, value =
        "SELECT * FROM tb_course ORDER BY created_at DESC;")
    List<Course> findRecentCourses();

    @Query(nativeQuery = true, value =
        "SELECT * FROM tb_course WHERE tb_course.name LIKE CONCAT('%', :name, '%')")
    ...
    @Query(nativeQuery = true, value = """
        SELECT 
            ...
            tb_course.name, 
            tb_course.synopsis, 
            tb_course.thumbnail_url, 
            tb_course.featured,
            tb_course.category_id,
            tb_course.created_at,
            tb_course.updated_at,
